...
| --- | --- |
| App starts on x64 Linux | The self-contained package job runs `--version`, starts the real Avalonia desktop process for five seconds, and archives the same directory. |
...
| Save/load and target profiles are reliable | The end-to-end scenario saves the document envelope, reopens it, and checks the target profile and Unicode mappings. Legacy direct schema-v1 files remain covered. |
| Invalid projects produce actionable diagnostics | The `ErrorPath` facet covers malformed/future files, missing profiles, target compatibility, verifier failures, blocked output, and cancellation. |
| A valid project produces deterministic XKB v1 symbols | Linux unit/golden tests and the application end-to-end scenario verify stable symbols and manifests. |
...
| Integration CI is green | The final gate depends on the managed, XKB integration, and packaging jobs. |
| Documentation matches behavior | README and project-format, architecture, Linux, packaging, versioning, diagnostics, theming, and testing guides describe the shipped workflow. |
...
- open the packaged application on a supported x64 Linux desktop;
...
- generate a symbols component and confirm the reported artifact path holds it;
...
Record the tested OS versions and any waived limitation in the release notes. Desktop
registration/activation, signing, and installers remain outside the MVP and must not be represented
as completed by this checklist.
