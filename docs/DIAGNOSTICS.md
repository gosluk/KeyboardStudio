...
## Build and artifact diagnostics

| Code | Severity | Meaning |
|---|---|---|
| `GEN_SOURCE` | Error | The selected backend could not translate or generate source for the project. |

Backend-specific artifact diagnostics are declared by the backend that raises them; the codes the
Linux backend uses are listed below.
...
platform's file format: a source for any other layout format would drop dead keys and fifth levels
for exactly the same reason, and one shared range keeps two sources from assigning one number two
meanings. The wording is correspondingly source-neutral.
...
Continuous validation never invokes source generation, an external tool, or filesystem I/O. If later
rules become expensive, they must move behind a debounced or explicit validation boundary rather than
making editor input block on build work.
